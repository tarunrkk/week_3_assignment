...
class GenerationConfig:
    ...
    use_reranker: bool = True  # Whether to use reranker
...
class RAGGenerator:
    ...
    def _format_context(self, results: list[RetrievalResult]) -> str:
        ...
        """
        #1. Build a list of formatted source strings
        formatted_source_strings = []

        #2. For each result (enumerate with index starting at 1):
        for i, result in enumerate(results):
            formatted_source_strings.append(f'''
            --- Source {i+1} ---
            Title: {result.title}
            Authors: {result.authors}
            Section: {result.chunk_section}
    ...
    def generate(self, query: str, top_k: Optional[int] = None, return_sources: bool = True) -> dict:
        ...
        # 2. Retrieve relevant chunks:
        results = self.retrieval.retrieve(refined_query, top_k=top_k)
